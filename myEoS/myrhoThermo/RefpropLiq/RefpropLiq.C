...
Foam::RefpropLiq<Specie>::RefpropLiq(Istream& is)
:
    Specie(is),
    Tc_(readScalar(is)),
    Vc_(readScalar(is)),
...
{
    is.check("RefpropLiq<Specie>::RefpropLiq(Istream& is)");
}


template<class Specie>
Foam::RefpropLiq<Specie>::RefpropLiq
(
...
template<class Specie>
void Foam::RefpropLiq<Specie>::write(Ostream& os) const
{
    Specie::write(os);
}
...
)
{
    ...
    os.check
    (
        "Ostream& operator<<(Ostream& os, const RefpropLiq<Specie>& st)"
    );
    return os;
}
