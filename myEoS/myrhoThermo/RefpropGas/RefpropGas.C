...
Foam::RefpropGas<Specie>::RefpropGas(Istream& is)
:
    Specie(is),
    Tc_(readScalar(is)),
    Vc_(readScalar(is)),
...
{
    is.check("RefpropGas<Specie>::RefpropGas(Istream& is)");
}


template<class Specie>
Foam::RefpropGas<Specie>::RefpropGas
(
...
template<class Specie>
void Foam::RefpropGas<Specie>::write(Ostream& os) const
{
    Specie::write(os);
}
...
)
{
    ...
    os.check
    (
        "Ostream& operator<<(Ostream& os, const RefpropGas<Specie>& st)"
    );
    return os;
}
